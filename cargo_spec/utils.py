...
def AddExcelToSpec(request,specification):
    ...
    for i in range(1,len(dataframe.values)+1):
        ...
        suma+=float(dataframe.loc[i].at['value'])
        for item in list('ABCDEFG'):
            ws['{}{}'.format(item,actual_row)].border=thin_border
            ws['{}{}'.format(item,actual_row)].alignment=Alignment(horizontal='center')
    ...
    ws['F{}'.format(actual_row)]=f'{str(suma)}'
    ws['D{}'.format(actual_row)]='Suma'
    for i in list('DEFG'):
        ws['{}{}'.format(i,actual_row)].border=thin_border
